...
W tym zagadnieniu szczególnie istotne są niektóre triki stosowane w dowodach.
Funkcja $f$ ograniczona przez funkcję $g$ ($f(x) \leq g(x)$ dla $x \in A$),
co implikuje, że $\lim_{x \to x_0} f(x) \leq \lim_{x \to x_0} g(x)$.
Również, bardzo łatwo udowodnić brak istnienia granicy, chociażby poprzez
kontrprzykład. Wystarczy znaleźć dwa ciągi o różnej wartości granicznej $f$.
$(\frac{1}{n}, 0)$ i $(\frac{1}{n}, \frac{1}{n})$  to bezpieczne przykłady.
Warto też wspomnieć, o uproszczeniu przez ograniczenie, gdzie dla funkcji
wielu zmiennych $f(x, y)$, definiujemy $u_n = \max\{x_n, y_n\}$, upraszczamy
wzór funkcji i w szczególnych przypadkach pozwala nam to na uproszczenie
dowodu.
...
\subsubsection{Współrzędne biegunowe}

$$
D = \{(r \cos \alpha, r \sin \alpha): r_1 \leq r \leq r_2; 0 \leq
\alpha_1 \leq \alpha \leq \alpha_2 \leq 2\pi\}
$$

$$
\iint_D f(x, y) \, dx \, dy = \iint_{[r_1, r_2] \times [\alpha_1,
\alpha_2]} f(r \cos \alpha, r \sin \alpha) \, r \, dr \, d\alpha
$$
