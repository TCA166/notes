...
Postawienie kolejnego urządzenia SG, pod kątem $\theta$, obniża
prawdopodobieństwo
pomiaru spinu w osi X na $\cos^2(\frac{\theta}{2})$.
...
\subsection{Opis eksperymentu Sterna-Gerlacha algebraiczny}

Załóżmy, że $\ket{\uparrow_z} =
\begin{pmatrix} 1 \\ 0
\end{pmatrix}$ i $\ket{\downarrow_z} =
\begin{pmatrix} 0 \\ 1
\end{pmatrix}$.
$$
S_z \ket{\uparrow_z} = \lambda_z \ket{\uparrow_z} , \quad S_z
\ket{\downarrow_z} = -\lambda_z \ket{\downarrow_z}
$$
zazwyczaj definiujemy $\lambda_z = \frac{\hbar}{2}$
$$
\braket{\uparrow_z}{\downarrow_z} = 0
$$

Operatory spinu muszą spełniać relacje komutacyjne:
$$
[S_x, S_y] = i\hbar S_z, \quad [S_y, S_z] = i\hbar S_x, \quad [S_z,
S_x] = i\hbar S_y
$$

Powyższe, pozwala nam określić $S_z$, a także $S_x$, $S_y$. Tak się składa, że
te macierze to macierze Pauliego; $\sigma_x$, $\sigma_y$, $\sigma_z$.
$$
\sigma_x =
