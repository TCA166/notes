...
Języki w zależności od swoich właściwości można podzielić na klasy.
Z reguły języki dzielimy na klasy w zależności od tego co jest konieczne
do ich rozpoznania i generowania.
...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
    \node[state,initial, accepting] (q_0)   {$q_a$};
    \node[state,accepting] (q_1) [right=of q_0] {$q_b$};
  ...
  \end{tikzpicture}
  \caption{Konstrukcja automatu deterministycznego dla języka $L(a^*b^*)$}
  \label{fig:cls:ex1}
\end{figure}
...
\begin{figure*}[h]
  ...
  \begin{tikzpicture}
    ...
    ({\x}, {0.5 * \x * \x + 1}) node[right] {$f(x)$};
    \draw[domain=0:2, smooth, variable=\x, red] plot
    ({\x}, {\x * \x}) node[right] {$g(x)$};
    \draw[dashed] (1.414213562, 0) -- (1.414213562, 4) node[above] {$n_0$};
  \end{tikzpicture}
