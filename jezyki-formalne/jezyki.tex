...
\subsubsection{Przekształcenie \texorpdfstring{$\epsilon \rightarrow$}{eps ->} ndet}
...
\begin{figure}[h]
    \centering
    \begin{tikzpicture}[shorten >=1pt,node distance=1cm,on grid,auto,rounded corners=0.5cm]
...
$$
S \stackrel{*}{\Rightarrow} a^{2n}SA^n \Rightarrow a^{2n}A^n \stackrel{*}{\Rightarrow} a^{2n}b^m
$$
...
$$
S \stackrel{*}{\Rightarrow} (ab)^nS(cd)^n \Rightarrow (ab)^nA(cd)^n \stackrel{*}{\Rightarrow} (ab)^n(dc)^mA(ba)^m(cd)^n \Rightarrow (ab)^n(dcba)^m(cd)^n
$$
