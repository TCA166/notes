...
szyfrowanie wiadomości $m$ przy pomocy ustalonego sekretu $s =
g^{\alpha\beta}$ w następujący sposób:
$$
c = m \cdot s
$$
$$
m = c \cdot s^{-1}
$$
Aby zapewnić bezpieczeństwo, należy wymieniać $s$, czyli również $\alpha, \beta$
za każdą wiadomością.
...
kryptoanalityzcne. 64 bitowy klucz to po prostu za mało, zwłascza, że
tak naprawdę
klucz ma 56 bitów. $2^{56} \sim 10^{16} $, co więcej, klucz DES
mieści się w jednym rejestrze w architekturach 64-bitowych
i przeszukanie wszystkich kluczy wiąże się z jedną pętlą \texttt{for} na type
\texttt{uint64\_t}.
