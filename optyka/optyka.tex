...
\subsection{Równanie falowe}

$$
\frac{\partial^2 f}{\partial z^2} = \frac{1}{v^2} \frac{\partial^2
f}{\partial t^2}
$$
Ogólnym rozwiązeniem tego równania, jest funkcja postaci:
$$
f(z, t) = g(z - vt)
$$
...
Fala płaska to taka, której czoło fali jest płaszczyzną, o pewnym
kierunku $\vec{v}$, i stałym kierunku propagacji $\vec{k}$.
$$
\psi(\vec{v}, t) = A f(k \vec{v} - \omega t)
$$

\subsection{Fala monochromatyczna}

Fala monochromatyczna, to taka, która ma jedną konkretną barwę, czyli jedną
...
\begin{figure}[h!]
  \centering
  \begin{tikzpicture}
    \draw[dotted, ->] (0,-0.5) -- (0,2) node[above] {$y$};
    \draw[dotted, ->] (0,0) -- (3,0) node[right] {$x$};

    \draw[->, thick] (1,1.5) node[above left] {$\vec{E}$} -- (2.5,0);
    \draw[->, dotted] (1,1.5) -- (2.5,1.5) node[above right]
    {$\vec{E}^\parallel$};
    \draw[->, dotted] (1,1.5) -- (1,0) node[above right] {$\vec{E}^\perp$};
  \end{tikzpicture}
  \caption{Składowe elektromagnetyczne fali padającej z ośrodka 1}
\end{figure}

Dla składowych prostopadłych do powierzchni $\perp$ oraz $\parallel$:
\begin{enumerate}
  \item $E_1^\parallel - E_2^\parallel = 0$ - pole elektryczne
  \item $D_1^\perp - D_2^\perp = 0$ - indukcja pola elektrycznego
  \item $H_1^\parallel - H_2^\parallel = 0$ - pola magnetyczne
  \item $B_1^\perp - B_2^\perp = 0$ - indukcja pola magnetycznego
\end{enumerate}
...
to kąt padania, a $\theta_2$ to kąt załamania. Prawo Snella pozwla określić
kąty załamania i padania, oraz współczynniki załamania, lecz nie pozwala:
\begin{itemize}
...
\begin{examplebox}
  ...
  \frac{1}{3} \sin(30^\circ) =  \sin(\theta_2)
  $$
